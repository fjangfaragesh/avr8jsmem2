...
export class MemOutElement extends HTMLElement implements MemOut {
  static observedAttributesArray = [
    "type",
    "address",
    "bytes",
    "endian",
  ...
  _typeStr = "hex";
  observedAttributesValues: {};
  memOutType: MemOutType;
  _address: string;
  ...
  setMemOutType(m) {
    ...
    for (let k of kys)
      m.attributeChangedCallback(k, null, this.observedAttributesValues[k]);
  }
...
class MemOutTypeText extends MemOutType {
  _addressStr: string = "no address";
  _address: number = -1;
  _bytes: number = 1;
  _endian: string = "little";
  _valueStr: string = "unknown";
  _outputType: string;

  constructor(elm: HTMLElement, outputType: string) {
    super(elm);
  ...
  showValue() {
    ...
    this.elm.appendChild(
      document.createTextNode(addrText + ": " + this._valueStr)
    );
  }
  attributeChangedCallback(name: string, oldValue: string, newValue: string) {
    ...
    if (name === "bytes") this._bytes = +newValue;
...
class MemOutTypeTextCustom extends MemOutType {
  _valueStr: string = "";
  _outputFunction: (data: Uint8Array, currentCycle: number) => string;
  constructor(elm) {
    super(elm);
...
class MemOutTypeDiagram2 extends MemOutType {
  ...
  _max: any = "auto";
  _color = "#000000";
  _outputsFunction: (data: Uint8Array, currentCycle: number) => any;
  _eChart: echarts.ECharts;
  _interval: number = 50000000;
  _title: string = "";
